...
void RGB2YUV422(unsigned short *rgb, unsigned short **yy, unsigned short **cbb,
                unsigned short **crr, const int NR, const int NC,
...
void RGB2YCbCr(
  ...
  for (int ii = 0; ii < nr * nc; ii++) {
    for (int icomp = 0; icomp < 3; icomp++) {
      ...
      *(ycbcr + ii + icomp * nr * nc) = (unsigned short) *(ycbcrD + ii
          + icomp * nr * nc);
    }
...
void YCbCr2RGB(
  ...
  for (int ii = 0; ii < nr * nc; ii++) {
    ...
    for (int icomp = 0; icomp < 3; icomp++) {
      ...
      *(rgb + ii + icomp * nr * nc) = (unsigned short) clip(
